...
class TestsAlza:
    ...
    def test_login_logout(self):
        ...
        assert actual_blank_email_text == TestData.blank_email_text, f"Wrong message for blank e-mail input field. Actual message is '{actual_blank_email_text}' but it shall be '{TestData.blank_email_text}'."
        actual_blank_password_text = self.login_page.login_get_blank_password_text()
        assert actual_blank_password_text == TestData.blank_password_text, f"Wrong message for blank password input field. Actual message is '{actual_blank_password_text}' but it shall be '{TestData.blank_password_text}'."
        ...
        assert actual_disabled_login_button_text == TestData.signin_button_incorrect_user_name_password_text, f"Wrong message at signin button when incorrect e-mail provided. Actual message is '{actual_disabled_login_button_text}' but it shall be '{TestData.signin_button_incorrect_user_name_password_text}'."
        ...
        assert self.login_page.login_dialog_is_visible(), "Login dialog is not visible though it shall be."
        actual_disabled_login_button_text = self.login_page.login_get_disabled_login_button_text()
        assert actual_disabled_login_button_text == TestData.signin_button_incorrect_user_name_password_text, f"Wrong message at signin button when incorrect password provided. Actual message is '{actual_disabled_login_button_text}' but it shall be '{TestData.signin_button_incorrect_user_name_password_text}'."
        ...
        assert actual_signed_in_text == TestData.user_signed_in_text, f"Wrong text in the top menu. Actual text is '{actual_signed_in_text}' but shall be '{TestData.user_signed_in_text}'. Seems user is not logged in though shall be."
    ...
    def test_basket_add_remove_item(self):
        ...
        assert first_computer_name in actual_computer_name, f"Wrong name of computer in basket. Actual name is '{actual_computer_name}' but it shall be '{first_computer_name}'. Seems computer that shall be in basket is not."
        ...
        assert actual_computer_price_in_basket == first_computer_price, f"Wrong computer price in basket. Actual price is '{actual_computer_price_in_basket}' but shall be '{first_computer_price}'."
        ...
        assert actual_text_once_basket_empty == TestData.text_once_all_items_removed_from_basket, f"Wrong text once basket is empty. Actual text is '{actual_text_once_basket_empty}' but it shall be '{TestData.text_once_all_items_removed_from_basket}'. Seems basket is not empty."
    ...
    def test_search(self):
        ...
        assert actual_search_result_title == TestData.search_result_header_via_search_button, f"Wrong header of looked up section is displayed. Actual header is '{actual_search_result_title}' but it shall be '{TestData.search_result_header_via_search_button}'. Seems wrong section is displayed."
        ...
        assert TestData.search_result_word_in_title_via_suggestion in actual_search_result_title, f"Result doesn't contain looked up word in title. Actual title is '{actual_search_result_title}', it does not contain word '{TestData.search_result_word_in_title_via_suggestion}' though it shall."
    ...
    def test_watchdog_add_remove_item(self):
        ...
        assert actual_email == TestData.user_name, f"Incorrect e-mail address prefilled. There is '{actual_email}' but there shall be '{TestData.user_name}'."
        # Set watch price.
        ...
        assert first_pet_supply_name in actual_pet_supply_name, f"Wrong name of pet supply in watchdogs. Actual pet supply name is '{actual_pet_supply_name}' but it shall be '{first_pet_supply_name}'. Seems pet supply that shall be in watchdogs is not."
        actual_price_limit = self.watchdogs_page.watchdogs_get_price_limit_provided()
        assert actual_price_limit == TestData.watchdog_price_limit, f"Wrong price limit displayed in watchdogs. Actual price limit is '{actual_price_limit}' but it shall be '{TestData.watchdog_price_limit}'."
        ...
        assert actual_text_once_watchdog_list_empty == TestData.text_once_all_items_removed_from_watchdog_list, f"Wrong text once watchdog list is empty. Actual text is '{actual_text_once_watchdog_list_empty}' but it shall be '{TestData.text_once_all_items_removed_from_watchdog_list}'. Seems watchdog list is not empty."
    ...
    def test_add_remove_delivery_addresses(self):
        ...
        assert actual_number_of_addresses == expected_number_of_delivery_addresses, f"Incorrect number of delivery addresses. Actual number is '{actual_number_of_addresses}' but it shall be '{expected_number_of_delivery_addresses}'."
        ...
        assert actual_delivery_addresses_data == TestData.delivery_addresses_original, f"Actual delivery addresses are not the same as provided delivery addresses. Actual delivery addresses are '{actual_delivery_addresses_data}', expected delivery addresses are '{TestData.delivery_addresses_original}'."
        ...
        actual_delivery_addresses_data = self.delivery_addresses_page.delivery_addresses_get_addresses_data(actual_number_of_addresses)
        assert actual_delivery_addresses_data == TestData.delivery_addresses_edited, f"Actual delivery addresses are not the same as updated delivery addresses. Actual delivery addresses are '{actual_delivery_addresses_data}', expected delivery addresses are '{TestData.delivery_addresses_edited}'."

        # Remove delivery addresses:
        self.delivery_addresses_page.delivery_addresses_remove_all_addresses_from_delivery_addresses_list()
        # Check there are no addresses.
        actual_number_of_addresses = self.delivery_addresses_page.delivery_addresses_get_number_of_addresses()
        assert actual_number_of_addresses == 0, f"There are delivery addresses present though there shall not be any. There are '{actual_number_of_addresses}' delivry addresses."
