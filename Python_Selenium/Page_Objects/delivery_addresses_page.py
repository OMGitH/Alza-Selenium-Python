...
class DeliveryAddresses(BasePage):
	...
	def delivery_addresses_remove_all_items(self, number_of_checks=10, check_wait=0.5):
		if self.base_is_visible(self.delivery_address_item, 3):
			while self.base_is_visible(self.delivery_address_item_remove_button, 1):
				...
				if number_of_items != 0:
					...
					# try:
					# 	number_of_items_after_removal = self.base_get_number_of_elements(self.delivery_address_item_remove_button, 2)
					# 	if number_of_items_after_removal == number_of_items - 1:
					# 		break
					# 	time.sleep(check_wait)
